...
function ChapterHeading({ index, title, detail }: { index: number; title: string; detail: string }) {
  ...
  return (
    <Link href={postHref(post)} className="mountain-post-row">
      ...
      <span className="mountain-post-thumb">
        {post.coverImage?.url ? (
          <Image src={post.coverImage.url} alt={post.coverImage.alt || post.title} width={160} height={120} className="mountain-post-image" unoptimized />
        ) : (
...
function ProjectNote({ project, labels, completed = false }: { project: HomeProjectCard; labels: HomeLabels; completed?: boolean }) {
  const progress = Math.max(1, Math.min(5, project.progress || 1));
  return (
    <Link href={project.category ? `/${project.category.slug}/${project.slug}` : '/'} className={`mountain-project-note ${completed ? 'is-completed' : ''}`}>
      ...
      <span className="mountain-progress" aria-label={`${labels.progress} ${progress}/5`}>
        {Array.from({ length: 5 }, (_, index) => <i key={index} className={index < progress ? 'is-filled' : ''} />)}
      </span>
    </Link>
  );
}
...
function LifeJournalCard({ post, labels, index }: { post: SpecialBlogItem; labels: HomeLabels; index: number }) {
  return (
    <Link href={postHref(post)} className={`mountain-life-card mountain-life-card-${(index % 5) + 1}`}>
      <span className="mountain-life-media">
        {post.coverImage?.url ? (
          <Image src={post.coverImage.url} alt={post.coverImage.alt || post.title} width={720} height={500} className="mountain-life-image" unoptimized />
        ) : (
...
function RamblingNote({ post, labels, index }: { post: SpecialBlogItem; labels: HomeLabels; index: number }) {
  ...
  return (
    <Link href={postHref(post)} className="mountain-rambling-note">
      ...
      <h3>{post.title}</h3>
      {(post.excerpt || post.bodyText) && <p>{post.excerpt || excerptText(post.bodyText)}</p>}
...
function CampEntry({ entry, labels, index }: { entry: HomeEntryCard; labels: HomeLabels; index: number }) {
  return (
...
export function HomeDashboard({ payload, motto, labels }: { payload: HomePayload; motto?: MottoDoc; labels: HomeLabels }) {
  ...
  const specialPosts = specialSeed === 0 ? payload.specialPosts.slice(0, 5) : pickRandom(payload.specialPosts, 5, specialSeed);
  const completedProjects = completedSeed === 0 ? payload.completedProjects.slice(0, 3) : pickRandom(payload.completedProjects, 3, completedSeed);
  const lifeRecentPosts = lifeSeed === 0 ? payload.lifeRecentPosts.slice(0, 5) : pickRandom(payload.lifeRecentPosts, 5, lifeSeed);
    ...
    <main className="mountain-home">
      ...
      <section id="base-camp" className="mountain-chapter mountain-base-camp" aria-labelledby="mountain-site-title">
        ...
        <div className="mountain-base-layout">
          <div className="mountain-hero-copy">
            ...
            <a href="#technical-ridge" className="mountain-next-link"><span>{labels.nextCamp}</span><i aria-hidden="true">↓</i></a>
          </div>

          <div className="mountain-dispatch-panel">
            <div className="mountain-section-bar">
              <div><span>{labels.dispatch}</span><h2>{labels.featured}</h2></div>
              <button type="button" className="mountain-icon-button" title={labels.refresh} aria-label={labels.refresh} onClick={() => setSpecialSeed((seed) => seed + 1)}>↻</button>
            </div>
            <div className="mountain-post-list">
              {specialPosts.length > 0 ? specialPosts.map((post, index) => <PostLogRow key={post._id} post={post} labels={labels} index={index} />) : <p className="mountain-empty-note">{labels.noPosts}</p>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="technical-ridge" className="mountain-chapter mountain-technical-ridge" aria-labelledby="technical-ridge-title">
        <div className="mountain-chapter-inner">
          <ChapterHeading index={1} title={labels.chapterNames[1]} detail={`${labels.skills} / ${labels.ongoingProjects} / ${labels.completedProjects}`} />
          <div className="mountain-ridge-layout mountain-ridge-route-map">
            <section className="mountain-skill-route" aria-labelledby="technical-ridge-title">
              ...
              <div className="mountain-subheading"><span>ANCHORS / 01</span><h3 id="technical-ridge-title">{labels.skills}</h3></div>
              <div className="mountain-skill-list">{payload.skillCategories.map((category, index) => <SkillMarker key={category._id} category={category} index={index} />)}</div>
            </section>
            <section className="mountain-project-route" aria-labelledby="ongoing-projects-title">
              <div className="mountain-subheading"><span>ASCENT / 02</span><h3 id="ongoing-projects-title">{labels.ongoingProjects}</h3></div>
              <div className="mountain-project-stack">{payload.ongoingProjects.slice(0, 5).map((project) => <ProjectNote key={project._id} project={project} labels={labels} />)}</div>
            </section>
            <section className="mountain-summit-log" aria-labelledby="completed-projects-title">
              <div className="mountain-subheading">
                <span>FIXED POINTS / 03</span><h3 id="completed-projects-title">{labels.completedProjects}</h3>
                <button type="button" className="mountain-icon-button" title={labels.refresh} aria-label={labels.refresh} onClick={() => setCompletedSeed((seed) => seed + 1)}>↻</button>
              </div>
              <div className="mountain-completed-grid">{completedProjects.map((project) => <ProjectNote key={project._id} project={project} labels={labels} completed />)}</div>
            </section>
          </div>
        </div>
      </section>
      ...
      <section id="snowfield-traverse" className="mountain-chapter mountain-snowfield" aria-labelledby="snowfield-title">
        <div className="mountain-chapter-inner">
          <ChapterHeading index={2} title={labels.chapterNames[2]} detail={`${labels.life} / ${labels.lifeRecent}`} />
          <div className="mountain-snowfield-layout mountain-snowfield-observation-band">
            <section className="mountain-life-signs" aria-labelledby="snowfield-title">
              <div className="mountain-subheading"><span>TRAIL MARKERS</span><h3 id="snowfield-title">{labels.life}</h3></div>
              <div className="mountain-sign-list">{payload.lifeCategories.map((category, index) => <LifeSign key={category._id} category={category} index={index} />)}</div>
            </section>
            <section className="mountain-life-journal" aria-labelledby="life-journal-title">
              <div className="mountain-section-bar">
                <div><span>OBSERVATION STRIP</span><h3 id="life-journal-title">{labels.lifeRecent}</h3></div>
                <button type="button" className="mountain-icon-button" title={labels.refresh} aria-label={labels.refresh} onClick={() => setLifeSeed((seed) => seed + 1)}>↻</button>
              </div>
              <div className="mountain-life-grid">{lifeRecentPosts.map((post, index) => <LifeJournalCard key={post._id} post={post} labels={labels} index={index} />)}</div>
            </section>
          </div>
        </div>
      </section>
      ...
      <section id="night-camp" className="mountain-chapter mountain-night-camp" aria-labelledby="night-camp-title">
        ...
        <div className="mountain-chapter-inner">
          <ChapterHeading index={3} title={labels.chapterNames[3]} detail={`${labels.ramblings} / ${payload.entryCards.map((entry) => labels.entryKinds[entry.kind]).join(' / ')}`} />
          <div className="mountain-night-layout">
            <section className="mountain-field-notes" aria-labelledby="night-camp-title">
              <div className="mountain-subheading"><span>{labels.fieldNotes}</span><h3 id="night-camp-title"><Link href="/daily-ramblings">{labels.ramblings}</Link></h3></div>
              <div className="mountain-note-list">{payload.ramblingPosts.slice(0, 3).map((post, index) => <RamblingNote key={post._id} post={post} labels={labels} index={index} />)}</div>
            </section>
            <section className="mountain-camp-map mountain-constellation" aria-label={labels.route}>
              <span className="mountain-camp-light" aria-hidden="true" />
              <div className="mountain-entry-grid">{payload.entryCards.map((entry, index) => <CampEntry key={entry._id} entry={entry} labels={labels} index={index} />)}</div>
            </section>
          </div>
        </div>
        <footer className="mountain-home-footer"><span>iceaxing</span><span>3200m → 6200m</span><a href="#base-camp">↑ TOP</a></footer>
      </section>
    </main>
  );
}
